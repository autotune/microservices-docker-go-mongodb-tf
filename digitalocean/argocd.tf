  ...
  spec {
    ...
    source {
      helm {
        ...
        parameter {
          name  = "customPlaybooks[0].actions.triggers[0].on_replicaset_update.included_types[1]"
          value = replace("Normal", "\"", "")
        }

        parameter {
          name  = "customPlaybooks[0].actions[0].resource_babysitter[0].fields_to_monitor[0]"
          value = "spec.replicas"
        }
